...
function extractLinks(options, body) {
  ...
  $("div")
    .find("a[jsname='UWckNb']")
    .each((i, elem) => {
      const titleElement = $(elem).find("h3.LC20lb");
      const url = $(elem).attr("href");

      if (url && titleElement.length) {
        const title = titleElement.text().trim();
        const ignore = options.ignore || [];

        if (!ignore.some((ignoreItem) => url.includes(ignoreItem))) {
          links.push({ url, title });
        }
      }
    });

  // Debug: Log part of the HTML body if no links are extracted
  if (links.length === 0) {
    console.log("No links extracted. HTML content:", body.substring(0, 500));
  }
